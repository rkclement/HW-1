...
as.data.frame(uniqDestinations)

uniqDestinations$state <- factor(uniqDestinations$state, levels = uniqDestinations$state)

g <- ggplot(filter(uniqDestinations, !is.na(state)), aes(x = state, y = n))
g + geom_bar(stat="identity") + coord_flip() + labs(title = "States to which Listed Southwest Flights Tend to Fly")
...
as.data.frame(allDestinations)

allDestinations$state <- factor(allDestinations$state, levels = allDestinations$state)

g <- ggplot(filter(allDestinations, !is.na(state)), aes(x = state, y = n))
g + geom_bar(stat="identity") + coord_flip() + labs(title = "States to which All Southwest Flights Tend to Fly")
...
*Fog-Rain and Rain-Thunderstorm are the weather patterns associated with the biggest departure delays, by far.*
...
*See chart of proportions below.*
